...
$realtime = mysql_fetch_array(mysql_query("SELECT * FROM setting"));
		...
		if($tgl==$tglsekarang){
			...
			if($nilaiC<>0) {
				...
				elseif($nilai['ujian_mulai']<>'' AND $nilai['ujian_selesai']=='') {
					...
					($mnt<>0) ? $lama .= "$mnt menit ":null;
					($dtk<>0) ? $lama .= "$dtk detik ":null;
					//$jawaban = "<small class='label bg-red'>Dikerjakan: $etong dari $mapel[jml_soal]</small><small class='label bg-green'>Benar: $nilai[jml_benar] <i class='fa fa-check'></i></small>  <small class='label bg-red'>Salah: $nilai[jml_salah] <i class='fa fa-times'></i></small>";
					//$skor = "<small class='label bg-green'>".number_format($nilai['skor'],2,'.','')."</small>";
