...
if($ac=='') {
														...
														while($siswa = mysql_fetch_array($siswaQ)) {
															...
															if($nilaiC<>0) {
																...
																if($nilai['ujian_mulai']<>'' AND $nilai['ujian_selesai']<>'') {
																	...
																	$jawaban = "<small class='label bg-green'>Betul: $nilai[jml_benar] <i class='fa fa-check'></i></small>  <small class='label bg-red'>Salah: $nilai[jml_salah] <i class='fa fa-times'></i></small>";
																	...
																	$ket = "<label class='label label-success'><i class='fa fa-check' title='Selesai ujian'></i>&nbsp;Selesai</label>";
																}
																	...
																	<td >
																		...
																		if($nilai['skor']<>"") {
																			...
																			if($pengawas['level']=='admin'){
																				...
																				if($ket<>""){
																				echo "
																					<a href='?pg=$pg&ac=selesai&idm=$id_mapel&idk=$id_kelas&ids=$siswa[id_siswa]' class='btn btn-xs btn-primary'>Selesaikan</a>
																				";
																				}
																			}
																			echo "
							...
							if (isset($_POST['simpanesai'])){
								...
								for ($i=1; $i <= $jml_data; $i++){
									...
									(!$exec) ? $info = info("Gagal menyimpan!","NO") : jump("?pg=nilai&ac=lihat&idm=$id_mapel&idk=$id_kelas");
								}
												...
												<div class='table-responsive'>
													<table  class='table table-bordered table-striped'>
														...
														while($jawaban = mysql_fetch_array($jawabanQ)) {
															...
															if($soal['file']==''){$gambar='';}else{$gambar="<img src='$homeurl/files/$soal[file]' class='img-responsive' style='max-width:300px;'/><p>";}
															if($soal['file1']==''){$gambar2='';}else{$gambar2="<img src='$homeurl/files/$soal[file1]' class='img-responsive' style='max-width:300px;'/><p>";}
								...
								<div class='row'>
									<div class='col-md-12'>
										<div class='box box-solid'>
											...
											<tr><th >Nama</th><td width='10'>:</td><td>$siswa[nama]</td><td rowspan='3' width='150' align='center' style='font-size:30px;vertical-align: middle;'>$nilai[total]</td></tr>
											...
											<tr><th >Mata Pelajaran</th><td width='10'>:</td><td>$namamapel[nama_mapel]</td></tr>
												...
												<table  class='table table-bordered table-striped'>
													...
													$soalee=mysql_fetch_array(mysql_query("select * from soal where id_soal='$jawabane[id_soal]'"));
